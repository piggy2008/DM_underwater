...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks2 = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(int(depth/2))
        ])

        self.zero_control = nn.ModuleList([
            nn.Conv1d(1024, 1024, 3, 1, 1) for _ in range(int(depth / 2))
        ])
    ...
    def initialize_weights(self):
        ...
        w_style = self.style_embedder.proj.weight.data
        nn.init.xavier_uniform_(w_style.view([w_style.shape[0], -1]))
        nn.init.constant_(self.style_embedder.proj.bias, 0)
...
if __name__ == '__main__':
    ...
    output = model(img, time, img, context)
    # output = model2(img)
    print(output.shape)
