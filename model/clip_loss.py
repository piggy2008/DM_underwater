...
class CLIPLoss(torch.nn.Module):
    ...
    def clip_directional_loss(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str) -> torch.Tensor:
        ...
        source_features_txt = self.get_text_features(source_class)
        target_features_txt = self.get_text_features(target_class)
        # source_features_txt /= source_features_txt.norm(dim=-1, keepdim=True)
        # target_features_txt /= target_features_txt.norm(dim=-1, keepdim=True)
        print(src_encoding.shape, '-----', source_features_txt.shape)
        src_fuse_features = torch.cat([src_encoding, source_features_txt], dim=-1)
        target_fuse_features = torch.cat([target_encoding, target_features_txt], dim=-1)
        value = torch.nn.CosineSimilarity()(src_fuse_features, target_fuse_features)
        # edit_direction = (target_encoding - src_encoding)
        # print('target_encoding:', src_encoding)
